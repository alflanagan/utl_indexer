...
class UTLParser(object):  # pylint: disable=too-many-public-methods
    ...
    def p_empty(self, p):
        '''empty :'''
        # you'd think we could just use None, but not good idea
        p[0] = ASTNode('empty', True)
    ...
    def p_param_list(self, p):
        '''param_list : param_decl
                      ...
                      | empty'''
        if p[1].symbol == 'empty':
            p[0] = p[1]  # why create another?
        elif p[1].symbol == 'param_decl':
    ...
    def p_method_call(self, p):
        ...
        p[0] = ASTNode('call', False, {'name': p[1]}, p[3])
